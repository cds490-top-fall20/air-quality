...
full_data<-full_data %>%
 ...
 full_data$`Pollutant Grade`<-as.factor(full_data$`Pollutant Grade`)
  ...
  full_data$`Annual Pass/Fail`<-factor(
    full_data$`Annual Pass/Fail`,c("PASS","INC","DNC"))

  
factpal<-colorFactor(c("forestgreen","gold","firebrick1"), levels(full_data[[input$Grade]]),na.color = "grey",ordered = T)
